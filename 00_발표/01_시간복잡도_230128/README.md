...
### QnA
  ...
  1. 피보나치 재귀함수는 왜 O( $2^n$ )인가요?
        ...
        > <br><br> 그러한 관점에서 f(n)은 항상 2^1의 연산을 수행하며 그 결과에 대한 f(n-1)도 2^1 f(n-2)도 2^1을 항상 수행함을 전제로 한다. 그렇기에 f(n)안에서 피보나치 2번 수행(=f(2)=2번), f(n)안의 2개의 피보나치에서 각 피보나치 1개당 피보나치 2번 수행 (=f(3)=2*2번), f(n)안의 피보나치 2개에서 각 2개씩 파생되고 그 4개의 피보나치에서 또 각 2번 수행 (=f(n)=2*2*...*2번)으로 f(n) = O(2^n)으로 생각할 수 있다.
        ><br><br>그렇기에 단순히 f(n)+f(n-1)로 생각하면 T(n) = c 2^1로 생각할 수 있지만 실상은 f(n) 안에 또 2번씩 시행되는 함수가 무한히 있는 형태이기에 트리의 높이인 k만큼 O(2^n)이라고 생각할 수 있는 것이다.
